  ...
  return (
    <>
      ...
      <section className="relative min-h-screen flex items-center pt-20 px-8 overflow-hidden bg-surface">
        ...
        <div ref={heroRef} className="animate-on-scroll relative z-10 max-w-7xl mx-auto w-full">
          <div className="max-w-2xl">
            <span className="font-sans text-sm uppercase tracking-[0.3em] text-tertiary mb-6 block">{t.home.heroTag}</span>
            <h1 className="font-serif text-[4rem] md:text-[5.5rem] leading-[1.1] text-on-surface tracking-tight mb-8">
              {t.home.heroTitle1} <br /><span className="italic text-primary">{t.home.heroTitle2}</span>
            </h1>
            <p className="text-lg md:text-xl text-on-surface-variant mb-12 max-w-lg leading-relaxed">{t.home.heroText}</p>
            <div className="flex flex-col sm:flex-row gap-6">
              <Link to="/contact" className="bg-primary text-on-primary px-10 py-5 rounded-full font-sans text-sm uppercase tracking-widest hover:scale-105 transition-all duration-300 bg-gradient-to-br from-primary to-primary-container shadow-xl shadow-primary/20 text-center">
                {t.home.bookClass}
              </Link>
              <Link to="/services" className="bg-white/40 backdrop-blur-md border border-outline-variant/30 text-on-surface px-10 py-5 rounded-full font-sans text-sm uppercase tracking-widest hover:bg-white/60 transition-all duration-300 text-center">
                {t.home.viewSchedule}
              </Link>
            </div>
          </div>
        ...
        <div className="absolute bottom-12 right-12 hidden lg:flex items-center space-x-4">
          ...
          <span className="font-serif italic text-primary">{t.home.breathe}</span>
        </div>
      </section>
          ...
          <div className="md:col-span-7 order-2 md:order-1">
            <div className="relative">
              ...
              <div className="absolute -bottom-10 -right-10 hidden lg:block w-64 h-64 bg-surface-container-low rounded-xl p-8 shadow-xl">
                ...
                <p className="font-serif italic text-on-surface text-lg">"The soul always knows what to do to heal itself."</p>
              </div>
            </div>
          </div>
          <div className="md:col-span-5 order-1 md:order-2">
            <h2 className="font-serif text-4xl md:text-5xl text-on-surface mb-8 leading-tight">{t.home.philTitle}</h2>
            <p className="text-on-surface-variant text-lg leading-relaxed mb-6">{t.home.philText1}</p>
            <p className="text-on-surface-variant text-lg leading-relaxed mb-10">{t.home.philText2}</p>
            <Link to="/about" className="flex items-center space-x-4 group cursor-pointer">
              <span className="font-sans text-sm uppercase tracking-widest text-primary border-b-2 border-primary-container pb-1 transition-all group-hover:pr-4">{t.home.ourStory}</span>
              <span className="material-symbols-outlined text-primary text-sm group-hover:translate-x-2 transition-transform">arrow_forward</span>
            </Link>
          </div>
        </div>
      </Section>
      ...
      <Section bg="low">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          <div>
            <span className="font-sans text-sm uppercase tracking-widest text-tertiary mb-4 block">{t.home.servicesTag}</span>
            <h2 className="font-serif text-4xl md:text-5xl text-on-surface leading-tight">{t.home.servicesTitle}</h2>
          </div>
        ...
        <div className="relative">
          <div className="flex overflow-x-auto gap-8 pb-8 snap-x snap-mandatory scrollbar-hide" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
            {[
              ...
              { to: "/services/healing-sessions", icon: "storm", title: t.home.healingTitle, text: t.home.healingText, b1: t.home.healingBullet1, b2: t.home.healingBullet2 },
      ...
      <section className="py-32 px-8 overflow-hidden">
        <div className="max-w-4xl mx-auto text-center relative">
          ...
          <div className="relative z-10">
            <p className="font-serif text-3xl md:text-4xl text-on-surface leading-snug mb-12 italic">{t.home.testimonialQuote}</p>
            <div className="flex items-center justify-center space-x-4">
              <img alt="Client portrait" className="w-16 h-16 rounded-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuBFxvd8HWpbXn1m6xqkOkT1aIxhfnHjqI-LuId9IyX87otiZa5GnkGcL5mgO0CAbVsGAFRzEqSl02Recvb7-UWC9tqQYWjPtsiDLZNtbeObGoAPHdH-WiIUOIZ_T83UONYAyG1vbuzZQ5813OrETTz5nlnGZXE0UBNPak3hPoq6a4N9pc6trBlAmJWtxIlAyPqPRXSuJ0ndJYGtpswW-fpaEpfgvUBlSF0VKVIg-h-oIttIvnI7_IWXoWN1AmUeJamxeGjz6_5zZGc" />
              <div className="text-left">
                <h4 className="font-sans font-bold text-on-surface uppercase tracking-widest text-sm">{t.home.testimonialName}</h4>
                <p className="text-tertiary text-xs uppercase tracking-widest">{t.home.testimonialRole}</p>
              </div>
            </div>
          </div>
        </div>
